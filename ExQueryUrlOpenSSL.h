...
SSLErrOut:
	...
	void InitFields()
	{
		SSLLastError.ctx = nullptr;
		SSLLastError.ssl = nullptr;
		SSLLastError.isVerClient = false;
		SSLLastError.Clear();
	}
...
//
//	bool SetCertificateInfo(const char * CertFile, int TypeCertFile = SSL_FILETYPE_PEM)
//	{
//		if(SSLLastError.ctx != nullptr)
//			SSL_CTX_free(SSLLastError.ctx);
//
...
//	}
	...
	)
	{
		...
		SSLLastError.ctx = SSL_CTX_new(SSLv3_method());
		if(SSLLastError.ctx == nullptr)
		{
			SSLLastError.Set();
			QUERY_URL::SetLastErr(EFAULT);
